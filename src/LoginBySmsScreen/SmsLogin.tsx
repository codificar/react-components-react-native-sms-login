...
const LoginBySms: React.FC<{
    ...
    const [isFocusedCellPhoneNumber, setIsFocusedCellPhoneNumber] = useState(false),
    ...
    function cellphoneValidation() {
            if (cellPhoneNumber !== '') {
                ...
                if (props.routeSendNumber !== '') {
                    axios.get(props.routeSendNumber, {
                        params: {
            ...
            function cellphoneValidationForUser() {
                if (cellPhoneNumber !== '') {
                    ...
                    if (cellPhoneNumber !== '') {
                        axios.post(props.routeSendNumber, {
                            phone: formattedNumberCell
                        }).then(response => {
                            setIsSendingCode(false)
                            responseRequestSendSms = response.data
            ...
            function validateCode() {
                ...
                if (props.routeSendSecCode !== '') {
                    axios.get(props.routeSendSecCode, {
                        params: {
            ...
            function validateCodeForUser() {
                ...
                if (props.routeSendSecCode !== '') {
                    axios.post(props.routeSendSecCode, {
                        user_id: providerId,
                        code: stringSecurity,
                    ...
                    function nextOrPrevInput(item, text) {
                        let arrayTextAux = []
                        ...
                        } else if (text !== '' && item < secCodeLenght - 1) {
                            this[`${item + 1}_ref`].current.focus()
                        } else if (text == '' && item == 1) {
                            this[`${item - 1}_ref`].current.focus()
                        }
                            ...
                            {!showInputSecCode ? (
                                <View /*style={{ alignItems: 'center' }}*/>
                                    <TextInputMask
                                        ...
                                        style={[styles.input, { borderBottomColor: isFocusedCellPhoneNumber ? '#6EB986' : '#6c757d' }]}
                                    ...
                                    />
                                    {emptyNumber ? (
                                        <Text style={styles.TextDesc}>{textDescription}</Text>
                                    ) : null}
                                    ...
                                    {isSendingCode ? (
                                        ...
                                        <TouchableOpacity style={buttonStyle.button}
                                            onPress={() => enviroment == 'provider' ? cellphoneValidation() : cellphoneValidationForUser()}>
                                            <Text style={styles.NextText}>{buttonSendtext}</Text>
                                        </TouchableOpacity>
                                    )}
                                </View>
                            ) : (
                                <View style={styles.ContValidation}>
                                    <Text style={styles.TextCodeSentTitle}>{codeSentTitle}</Text>
                                            ...
                                            (
                                                <TextInput style={styles.InputValidationCode}
                                                    ...
                                                    onKeyPress={({ nativeEvent }) => {
                                                        if (nativeEvent.key === 'Backspace' && arrayTexts[index] == '') {
                                    ...
                                    {isSendingCode ? (
                                        ...
                                        <TouchableOpacity style={styles.ButtonStyled}
                                            onPress={() => enviroment == 'provider' ? validateCode() : validateCodeForUser()}>
                                            <Text style={styles.NextText}>{props.buttonConfirmText}</Text>
                                        </TouchableOpacity>
                                    )}
                                </View>
                            )}
